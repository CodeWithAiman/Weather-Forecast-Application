...
const API_KEY = "723216b3d05a1440a2bb63cef103aa66"; // API key for OpenWeatherMap API
...
const getWeatherDetails = (cityName, latitude, longitude) => {
    ...
    fetch(WEATHER_API_URL).then(response => response.json()).then(data => {
        ...
        updateRecentCitiesDropdown(cityName);
...
const updateRecentCitiesDropdown = (cityName) => {
    let recentCities = JSON.parse(localStorage.getItem("recentCities")) || [];

    // Add new city to the list if not already present
    if (!recentCities.includes(cityName)) {
        recentCities.push(cityName);
        // Limit to the last 5 cities
    ...
    recentCities.forEach(city => {
        recentCitiesDropdown.insertAdjacentHTML("beforeend", `<option value="${city}">${city}</option>`);
    });
}
